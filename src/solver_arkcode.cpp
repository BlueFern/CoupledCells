		...
		int file_write_per_unit_time, int line_number, checkpoint_handle *check, char* path, IO_domain_info* my_IO_domain_info)
{
	...
	realtype T0 = tnow;      /* initial time */
	realtype Tf = tfinal;     /* final time */
	realtype dTout = interval;   /* time between outputs */
	long int NEQ = total;
	...
	y = N_VMake_Serial(NEQ, yInitial);  // No idea if this works. realdata == double ?
	...
	ARKodeSVtolerances(arkode_mem, reltol, atols);
