...
// DAG concentration for half maximum activation (mM). Corrected and compared with the JISM model.
double K_NSC = 3000e-6;
...
// Ca concentration in SR for half activation (mM).
double K_SOC = 100e-6;
...
// Cotransport coefficient (mmol^2/(J*s*cm^2)).
double L_NaKCl = 1.79e-11;

// Sarcoplasmic reticulum from Yang(2003).
...
// Maximum SR uptake current (pA) ranging between 6.68 - 20 pA.
double I_bar_SERCA = 20.0;

// Leakage parameter ranging between 1.07e-5 - 5.35e-5.
double R_leak = 5.35e-5;
...
// Dissociation constants for Ca activation and inhibitory sites (uM).
double K_actIP3 = 170e-6;
double K_inhIP3 = 100e-6;

// Dissociation constant for IP3 binding to IP3R (uM).
double K_IP3 = 120e-6;
...
// Set it to 0.1(1/ms) while replicating the results in Fig3 of (Tsoukias 2008).
double k_p_G = 0.0;
double deltaG_0 = 0;

// sGC activation and cGMP formation.
// sGC activation and inactivation time constants (ms).
double tau_a_sGC = 0.23e3;
double tau_d_sGC = 10.0e3;
