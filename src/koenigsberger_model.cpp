	...
	for (i = 1; i <= grid.num_smc_circumferentially; i++) {
		for (j = 1; j <= grid.num_smc_axially; j++) {
			...
			smc[i][j].A[J_CICR] = (CICRi * (P2(smc[i][j].p[smc_SR]) * P4(
					smc[i][j].p[smc_Ca]))) / ((P2(sci) + P2(smc[i][j].p[smc_SR]))
			* (P4(cci)+ P4(
							smc[i][j].p[smc_Ca])));
			//Jextrusion
			smc[i][j].A[J_Extrusion] = Di * smc[i][j].p[smc_Ca] * (1 + ((smc[i][j].p[smc_Vm] - vdi) / Rdi));

			//Jleak
			smc[i][j].A[J_Leak] = Li * smc[i][j].p[smc_SR];
			//Jvocc
			smc[i][j].A[J_VOCC] = GCai * (smc[i][j].p[smc_Vm] - vCa1) / (1 + ((double) (exp(((-1) * (smc[i][j].p[smc_Vm] - vCa2)) / RCai))));
			//J Na/Ca
			smc[i][j].A[J_Na_Ca] = GNaCai * smc[i][j].p[smc_Ca] * (smc[i][j].p[smc_Vm] - vNaCai) / (smc[i][j].p[smc_Ca] + cNaCai);
			//JNa/K
			smc[i][j].A[J_Na_K] = FNaK;
			//J Cl
			smc[i][j].A[J_Cl] = GCli * (smc[i][j].p[smc_Vm] - vCl);
			//JK
			smc[i][j].A[J_K] = GKi * smc[i][j].p[smc_w] * (smc[i][j].p[smc_Vm] - vKi);
			//Kactivation
...
void koenigsberger_ec_derivatives(double t, double* f, grid_parms grid, EC_cell** ec) {
	...
	for (int i = 1; i <= grid.num_ec_circumferentially; i++) {
		for (int j = 1; j <= grid.num_ec_axially; j++) {
			...
			f[k + ((j - 1) * grid.neq_ec) + ec_Ca] = ec[i][j].A[J_IP3] - ec[i][j].A[J_SERCA] + ec[i][j].A[J_CICR] - ec[i][j].A[J_Extrusion]
					+ ec[i][j].A[J_Leak] + ec[i][j].A[J_NSC] + ec[i][j].A[J_trivial_Ca] + ec[i][j].B[cpl_Ca] + ec[i][j].C[cpl_Ca];

			f[k + ((j - 1) * grid.neq_ec) + ec_SR] = ec[i][j].A[J_SERCA] - ec[i][j].A[J_CICR] - ec[i][j].A[J_Leak];

			f[k + ((j - 1) * grid.neq_ec) + ec_Vm] = ((-1 / Cmj) * (ec[i][j].A[J_Ktot] + ec[i][j].A[J_Residual])) + ec[i][j].B[cpl_Vm]
					+ ec[i][j].C[cpl_Vm];

			f[k + ((j - 1) * grid.neq_ec) + ec_IP3] = ec[i][j].JPLC - ec[i][j].A[J_IP3_deg] + ec[i][j].B[cpl_IP3] + ec[i][j].C[cpl_IP3];
		}
	}
}
