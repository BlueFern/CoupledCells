...
checkpoint_handle* initialise_checkpoint(grid_parms grid){
	...
	err=sprintf(filename,"time_%d.txt",grid.my_domain.internal_info.domain_index);
	CHECK(MPI_File_open(grid.sub_universe, filename, MPI_MODE_CREATE|MPI_MODE_RDWR, MPI_INFO_NULL, &check->Time));
...
void dump_smc(grid_parms grid, celltype1 **smc, checkpoint_handle *check, int write_count){
	...
	buffer = (double**) malloc(grid.num_smc_circumferentially * sizeof(double*));
	for(int i=1; i<=grid.num_smc_circumferentially; i++) {
		buffer[i] = (double*) malloc(grid.num_smc_axially* sizeof(double));
		}

	disp = time_offset_in_file + (grid.rank * write_element_count*sizeof(double));
	for(int i=1; i<=grid.num_smc_circumferentially; i++) {
		for(int j=1; j<=grid.num_smc_axially;j++) {
			buffer[i][j] = smc[i][j].p[smc_Ca];
		}
	}
...
void dump_ec(grid_parms grid, celltype2 **ec, checkpoint_handle *check, int write_count){
	...
	time_offset_in_file = write_count*write_element_count*grid.numtasks*sizeof(double);
	buffer = (double**) malloc(grid.num_ec_circumferentially * sizeof(double*));
	for(int i=1; i<=grid.num_ec_circumferentially; i++) {
		buffer[i] = (double*) malloc(grid.num_ec_axially* sizeof(double));
		}

	disp = time_offset_in_file + (grid.rank * write_element_count*sizeof(double));
	for(int i=1; i<=grid.num_ec_circumferentially; i++) {
		for(int j=1; j<=grid.num_ec_axially;j++) {
			buffer[i][j] = ec[i][j].q[ec_Ca];
		}
	}
...
void final_checkpoint(checkpoint_handle *check,double t1, double t2){
	MPI_Status	status;
	MPI_Offset	disp;
	double diff = t2-t1;
