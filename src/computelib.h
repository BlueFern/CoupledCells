...
typedef struct {
	...
	num_domains, **domains,
			...
			NEQ, num_fluxes_smc, num_fluxes_ec, num_coupling_species_smc,
...
//handlers specific for Tsoukias-SMC model variables
			...
			tsk_R_01, tsk_V_cGMP, tsk_cGMP, tsk_Na, tsk_K, tsk_Cl, tsk_DAG,
			tsk_PIP2, tsk_R_S_G, tsk_R_S_P_G, tsk_G,
...
void communication_update_sendbuf(grid_parms, double**, celltype1**,
		celltype2**);
void communication_update_recvbuf(grid_parms, double**, celltype1**,
		celltype2**);
...
void coupling(double, double*, grid_parms, celltype1**, celltype2**,
		conductance);
...
void checkpoint_with_ghost_cells(checkpoint_handle*, grid_parms, double,
		celltype1**, celltype2**, int);
...
double* reinitialize_koenigsberger_ec(checkpoint_handle*, int, grid_parms,
		double*, celltype2**);
int checkpoint(checkpoint_handle*, grid_parms, double*, double*, celltype1**,
		celltype2**);
