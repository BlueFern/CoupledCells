...
void write_HDF5_JPLC(grid_parms* grid, double *jplc_buffer, char *path, int timestep)
{
	...
	hsize_t dims[1] = {grid->num_ec_axially * grid->num_ec_circumferentially * grid->num_ranks_write_group};
	...
	dset_id = H5Dcreate(file_id, "data", H5T_NATIVE_DOUBLE, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
#else
	dset_id = H5Dcreate(file_id, "data", H5T_NATIVE_DOUBLE, space_id, H5P_DEFAULT);
#endif
...
void write_EC_data_HDF5(grid_parms* grid, double *ec_buffer, int write_count, char* path)
{
	...
	hsize_t dims[1] = {grid->num_ec_axially * grid->num_ec_circumferentially * (grid->num_coupling_species_ec + grid->neq_ec) * grid->num_ranks_write_group};
...
void write_SMC_data_HDF5(grid_parms* grid, double *smc_buffer, int write_count, char* path)
{
	...
	// Create a HDF5 file.
	file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	hsize_t dims[1] = {grid->num_smc_axially * grid->num_smc_circumferentially * (grid->num_coupling_species_smc + grid->neq_smc) * grid->num_ranks_write_group};
